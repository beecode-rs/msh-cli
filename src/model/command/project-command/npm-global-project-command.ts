...
export type VersionOnProjects = { [k: string]: string[] }
export type UniquePackages = { [k: string]: { versions: string[]; versionProject: VersionOnProjects } }
export type DependenciesObject = { [k: string]: string }
export type ProjectWithDependencies = { [k: string]: DependenciesObject }
...
export class NpmGlobalProjectCommand implements Executable {
  ...
  protected _removeIgnoredPackages(dependencies: DependenciesObject): DependenciesObject {
    ...
    if (globalIgnorePackages.length === 0) return dependencies
  ...
  protected _uniquePackages(depsByProject: ProjectWithDependencies): UniquePackages {
    ...
    Object.entries(depsByProject).forEach(([project, deps]) => {
      Object.entries(deps).forEach(([packageName, version]) => {
        ...
        if (!pk.versions.includes(version)) pk.versions.push(version)
  ...
  protected _highestVersion(versions: string[]): string {
    const sortedVersions = versions.sort(cVer)
    const result = sortedVersions.pop()
    if (!result) throw new Error(`Missing version in array [${stringify(versions)}]`)
    return result
  }
  ...
  protected _overrideGlobalDepsWithNewHighestDependencies(highestDependencies: DependenciesObject): void {
    ...
    fileService.writeToFileSync({ filePath: 'package.json', data: fileData })
  }
